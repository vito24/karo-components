...
import { Menu, Icon, Row, Col, Affix } from 'antd'; // TODO
...
import ComponentDco from './ComponentDoc';
...
function getModuleData(props) {
  ...
  const moduleName = /^\/?components/.test(pathname)
    ...
    : pathname
        ...
        .slice(0, 2)
        .join('/');
  const moduleData = ['components', 'docs/react', 'changelog', 'changelog-cn'].includes(moduleName)
  ...
  getMenuItems(footerNavIcons = {}) {
    ...
    return menuItems.map(menuItem => {
      if (menuItem.children) {
        return (
          <Menu.SubMenu title={<h4>{menuItem.title}</h4>} key={menuItem.title}>
      ...
      return this.generateMenuItem(false, menuItem, footerNavIcons);
    });
  }
  ...
  generateMenuItem(isTop, item, { before = null, after = null }) {
    ...
    const text = isTop
      ...
      : [
          ...
          <span key="chinese" className="chinese">
            {item.subtitle}
          </span>
        ];
    const { disabled } = item;
        ...
        <Row>
          ...
          <Col xxl={20} xl={19} lg={18} md={24} sm={24} xs={24}>
            <section className={mainContainerClass}>
              {props.demos ? (
                <ComponentDco {...props} doc={localizedPageData} demos={props.demos} />
              ) : (
